    ...
    $.ajax({
        ...
        success: function (cardData) {
            ...
            for (var i = 0; i < cardData.length; i++) {
                var cardImage = $('<img>').attr("src", cardData[i].preview);
                var cardTitle = $('<h6>').text(cardData[i].name);
                var cardBrand = $('<p>').text(cardData[i].brand);
                var cardPrice = $('<div>').text("Rs " + cardData[i].price);
                var j = i + 1;
                var cardDetails = $('<div>').attr('class', 'card_details').append(cardTitle, cardBrand, cardPrice);
                var cardBody = $('<a>').attr('class', 'card_body').append(cardImage, cardDetails);
